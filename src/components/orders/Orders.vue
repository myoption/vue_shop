<template>
  <div>
    ...
    <el-card>
      ...
      <el-row>
        <el-col :span="8">
          <el-input placeholder="请输入内容" v-model="queryInfo.query" class="search_input" @blur="getOrderList">
            <el-button slot="append" icon="el-icon-search" @click="getOrderList"></el-button>
          </el-input>
        </el-col>
      </el-row>
      <!-- 渲染订单数据表格 -->
      <el-table :data="goods" border stripe :highlight-current-row="true">
        ...
        <el-table-column label="操作">
          <template slot-scope="scope">
            <el-button type="primary" icon="el-icon-edit" size="mini" @click="showLocationDialog(scope.row)">编辑</el-button>
            <el-button type="success" icon="el-icon-location" size="mini" @click="showProgressDialog(scope.row)">物流</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 页码 -->
    ...
    <!-- 修改地址对话框 -->
    <el-dialog title="修改地址" :visible.sync="locationDialogVisible" width="40%" @close="addressDialogClose">
    ...
    <!-- 物流对话框 -->
    <el-dialog title="物流信息" :visible.sync="showProgressDialogVisible">
      <!-- 时间线 -->
...
export default {
  ...
  methods: {
    ...
    async showProgressDialog() {
      // console.log(order)
      /* const { data: result } = await this.$http.get('/kuaidi/80490957441254480')
      if (result.meta.status !== 200) {
